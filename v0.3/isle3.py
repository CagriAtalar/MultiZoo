...
def cutmix(images, labels, alpha=1.0):
    ...
    # Create mask
    mask = tf.ones((batch_size, img_height, img_width, 3))
    ...
    if x1_range > 0 and y1_range > 0:
        ...
        for i in range(batch_size):
            updates = tf.zeros((tf.shape(mask_indices)[0], 3))
...
callbacks = [
    ...
    create_lr_scheduler(),  # Custom LR scheduler with warmup
...
def train_with_mixup_cutmix(model, train_ds, val_ds, epochs, callbacks):
    ...
    for epoch in range(epochs):
        ...
        for metric in model.metrics:
            train_metrics[metric.name] = metric.result().numpy()
        ...
        val_metrics = {}
        for metric in model.metrics:
            val_metrics[metric.name] = metric.result().numpy()
            print(f"  val_{metric.name}: {metric.result().numpy():.4f}")
            metric.reset_states()
